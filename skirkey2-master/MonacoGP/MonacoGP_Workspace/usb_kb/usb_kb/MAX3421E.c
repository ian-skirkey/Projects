...
BYTE SPI_wr(BYTE data) {
	...
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0,
             1, &data,
             1, &usb_task_state,
             0);			 			//select MAX3421E (may not be necessary if you are using SPI peripheral)
	 	 	 	 	 	 	 	 	 	 //write data on SPI, simultanously read status register (byte) from MAX3421E
	 if (return_code < 0) {
		 	printf("Error during transfer"); 	 	 	 	 	 	 	 	//read return code from SPI peripheral (see Intel documentation)
	 }
	 	 	 	 	 	 	 	 	 	 //if return code < 0 print an error
	 	 	 	 	 	 	 	 	 	 //deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	 return usb_task_state;	 	 	 	 //return the status register
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
														//psuedocode:
	int i = 1;													//select MAX3421E (may not be necessary if you are using SPI peripheral)
	int p = 0;


		reg += 2;														//write reg + 2 via SPI
	...
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0,
		...
		/* for (i = 0; i < nbytes; i++) {
			 return_code = alt_avalon_spi_command(SPI_0_BASE, 1,
			 		             1, data,
			 		             0, 0,
			 		             0);
			 if (return_code < 0) {
			 	...
			 	data++;
														...
														//if return code < 0 print an error
														//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
		return (data + nbytes);												//return (data + nbytes);
}

//reads register from MAX3421E via SPI
BYTE MAXreg_rd(BYTE reg) {

	BYTE val = 0;																//psuedocode:
	...
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0,
		...
		return val;															//if return code < 0 print an error
																	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
																	//return val
}
//multiple-byte write
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
																					//psuedocode:
	int j;																				//select MAX3421E (may not be necessary if you are using SPI peripheral)
																					//write reg via SPI
