...
/* This program creates and operates on trees which represent
circuit designs. Namely, the internal nodes of the tree hold information 
about the cutlines, horizontal or vertical, between circuit modules (how they are oriented)
and the leaf nodes on the tree represent the modules themselves and hold details about them
such as their resource usage and their size */
...
if (ptr->left == NULL && ptr->right == NULL) {		//If the node does not have any children, then it is a leaf node
	return 1 ;
}
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
if (ptr->left != NULL && ptr->right != NULL) {		//If the node has both children, then it is an internal node
	return 1 ;
}  
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  if (a == b ) {	//If nodes are same then return 1
  	return 1 ;
  }
  
  if (is_leaf_node(a) == 0) {	//If node a is not a leaf, then recurse with both children of a and return their sum (1 if either one of them is b)
...
void rotate(node_t* ptr) {
  // TODO:
  int width, height ;		//Swap the width and height values in the given module
  width = ptr->module->w ;
  height = ptr->module->h ;
  ptr->module->w = height ;
  ptr->module->h = width ; 
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if (ptr->cutline == H) {	//If cutline is H, switch it to V and vice versa
  	ptr->cutline = V ;
  }
  else {
	ptr->cutline  = H ;
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t * temp = a->module ;		//Set each of the modules of the nodes equal to the other's module
  a->module = b->module ;
  b->module = temp ;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t * parenta = a->parent ;	//Get parent nodes for each given node
  node_t * parentb = b->parent ;
  
  if (parenta->right == a) {		//Find whether each node is on the left or right of its parent and change that pointer to point to the other node
  	
	parenta->right = b ;
	b->parent = parenta ;		//Then update the parent pointers of the swapped nodes
  }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  postfix_traversal (ptr->left, nth, expression) ; 	//Recurse function for children of the start node
  postfix_traversal (ptr->right, nth, expression) ;
  
  if (ptr->cutline == UNDEFINED_CUTLINE) {	//If node is a module, then set the postfix expression at the correct index to this module
  	expression[*nth].module = ptr->module ;
	expression[*nth].cutline = UNDEFINED_CUTLINE ;	//cutline is not defined for a module node
  }
  
  if (ptr->module == NULL) {
  	expression[*nth].module = NULL ;	//If node is a cutline, set the postfix expression at the correct index to the cutline for this node, and the module to NULL
	expression[*nth].cutline = ptr->cutline ;
  }
  (*nth)++ ;	//Increment the expression index
...
int get_total_resource(node_t* ptr)
{
  ...
  if (ptr->left == NULL) {		//If we have reached the final node, return its resource value
  	return ptr->module->resource ;
  }
  
  return ptr->right->module->resource + get_total_resource(ptr->left) ;	//Get resource of the right child for the current node and add it to the sum, and recurse to the next left child
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  if (n == num_modules-1) {	//If we have reached the final parent in the tree
  	node_t * left = (node_t*)malloc(sizeof(node_t)) ;
  	node_t * right = (node_t*)malloc(sizeof(node_t)) ;	//Allocate left and right child nodes
	right->right = NULL ;
	right->left = NULL ;	//Set their children to NULL
	left->right = NULL ;
	left->left = NULL ;
	par->left = left ;	//set parent pointers and children pointers
	par->right = right ;
	left->parent = par ;
	right->parent = par ;
	left->module = &(modules[n]) ;		//set the modules for the left and right child nodes and their cutlines (undefined)
  ...
  node_t * left = (node_t*)malloc(sizeof(node_t)) ;	//If we have not reached the end of the tree allocate a left child node
  left->parent = par ;
  left->cutline = V ;		//Set its parent, cutline, and module values
  left->module = NULL ;
  
  if (par != NULL) {		//If the node we created is not the head node, create a right child node
	node_t * right = (node_t*)malloc(sizeof(node_t)) ;
	right->parent = par ;		//set parent for right node pointer as well as both child pointers
	par->left = left ;
	par->right = right ;
	right->cutline = UNDEFINED_CUTLINE ;	//set right child cutline and module to appropriate values and its child pointers to NULL
	right->module = &(modules[n-1]) ;
	right->left = NULL ;
	right->right = NULL ;
  ...
  init_slicing_tree(left, n+1) ;	//Recurse with the left child node as the new parent and increment the module index counter
